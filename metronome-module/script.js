...
function playUIClick() {
    // Réactiver l'AudioContext si nécessaire (politique navigateur)
    ...
    try {
        ...
        // LATENCE 0MS : Jouer IMMÉDIATEMENT au currentTime exact (pas de délai)
        gainNode.gain.setValueAtTime(0.3, audioContext.currentTime);
        source.start(audioContext.currentTime); // Démarre instantanément
    } catch (error) {
...
function initTempoButtons() {
    ...
    if (plusBtn) {
        ...
        const handlePlus = () => {
            playUIClick();  // Son UI click IMMÉDIAT (0ms)
            
            // Feedback visuel classe .clicking (100ms)
        ...
        // ⚡ LATENCE 0MS : mousedown + touchstart (pas click)
        plusBtn.addEventListener('mousedown', handlePlus);
        plusBtn.addEventListener('touchstart', (e) => {
            e.preventDefault(); // Éviter le double-tap sur mobile
            handlePlus();
        });
    }
    
    if (minusBtn) {
        // Fonction MINUS (réutilisable)
        const handleMinus = () => {
            playUIClick();  // Son UI click IMMÉDIAT (0ms)
    ...
    if (tapBtn) {
        ...
        // Fonction TAP TEMPO (réutilisable)
        const handleTap = () => {
            playUIClick();  // Son UI click IMMÉDIAT (0ms)
            
            // FEEDBACK VISUEL: Ajouter classe .tapping pour texte blanc (100ms minimum)
            tapBtn.classList.add('tapping');
            setTimeout(() => {
                tapBtn.classList.remove('tapping');
            }, 100);  // 100ms = délai visible pour feedback blanc
        ...
        // LATENCE 0MS : Utiliser mousedown + touchstart (pas click qui a 300ms de délai)
        tapBtn.addEventListener('mousedown', handleTap);
        tapBtn.addEventListener('touchstart', (e) => {
            e.preventDefault(); // Empêcher le double-tap zoom sur mobile
            handleTap();
        }, { passive: false });
    }
}
...
function initUniversalUIClick() {
    // Add UI click sound to all interactive buttons that don't have it yet
    const selectors = [
        '.control-group',      // BEAT, BAR, NOTE selectors (changed from .control-value)
    ...
    if (stopBtn) {
        stopBtn.addEventListener('click', () => {
            if (isPlaying) {
                ...
                // Pas de flash glow - effet visuel retiré
