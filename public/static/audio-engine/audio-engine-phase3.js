/**
 * SHRED UP - Audio Engine (Phase 3 - Pitch Detection)
 * 
 * Main orchestrator for audio pipeline
 * Includes pitch detection consumer
 * 
 * Phase 3: Adds pitch detection to Phase 2A scaffolding
 */
...
class AudioEngine {
    ...
    async init() {
        try {
            ...
            logger.info('AUDIO-ENGINE', 'SHRED UP - Audio Engine Phase 3');
            logger.info('AUDIO-ENGINE', 'Pitch Detection Initialization');
            logger.info('AUDIO-ENGINE', '═══════════════════════════════════');

            // Step 1: Initialize frame buffer
            logger.info('AUDIO-ENGINE', 'Step 1/5: Initializing frame buffer...');
            frameBuffer.init();

            // Step 2: Initialize audio capture
            logger.info('AUDIO-ENGINE', 'Step 2/5: Initializing audio capture...');
            ...
            logger.info('AUDIO-ENGINE', 'Step 3/5: Initializing timing sync...');
            const timingInit = await timingSync.init();
            if (!timingInit) {
                throw new Error('Timing sync initialization failed');
            }

            // Step 4: Initialize pitch detection (Phase 3)
            logger.info('AUDIO-ENGINE', 'Step 4/5: Initializing pitch detection...');
            const pitchInit = pitchDetection.init();
            if (!pitchInit) {
                throw new Error('Pitch detection initialization failed');
            }
    ...
    /**
     * Start audio engine
     * Begins audio capture and pitch detection consumer
     */
    async start() {
        ...
        try {
            ...
            // Start pitch detection consumer loop
    ...
    /**
     * Start pitch detection consumer loop
     * Processes frames at ~46.88 Hz (every 21.33 ms)
     */
    startPitchConsumer() {
        logger.info('AUDIO-ENGINE', 'Starting pitch detection consumer...');

        // Consumer runs every 20ms (slightly faster than needed for 50% overlap)
        this.pitchConsumerInterval = setInterval(() => {
            ...
            // Result is logged by pitch detection module
            // No additional logging needed here

        }, 20); // 20ms = 50 Hz consumer rate

        logger.success('AUDIO-ENGINE', 'Pitch detection consumer started');
    }

    /**
     * Stop pitch detection consumer
     */
    stopPitchConsumer() {
        if (this.pitchConsumerInterval) {
            clearInterval(this.pitchConsumerInterval);
            this.pitchConsumerInterval = null;
            logger.info('AUDIO-ENGINE', 'Pitch detection consumer stopped');
        }
    }
    ...
    getStatus() {
        return {
            ...
            pitchDetection: pitchDetection.getStatus()
        };
    }
    ...
    logStatus() {
        ...
        // Formatted summary
        console.log('─────────────────────────────────');
        console.log('AUDIO ENGINE STATUS SUMMARY (Phase 3)');
        console.log('─────────────────────────────────');
        ...
        console.log(`Pitch Detect:  ${status.pitchDetection.initialized ? '✓ Active' : '✗ Inactive'}`);
        console.log('─────────────────────────────────');
        ...
        console.log(`Pitch Windows: ${status.pitchDetection.windowCount}`);
        console.log(`Avg Proc Time: ${status.pitchDetection.avgProcessingTime} ms`);
        console.log(`Max Proc Time: ${status.pitchDetection.maxProcessingTime} ms`);
        console.log('─────────────────────────────────');
    }
